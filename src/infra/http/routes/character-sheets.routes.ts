...
export async function characterSheetsRoutes(app: FastifyInstance) {
  ...
  app.post('/api/gm/sheets', app.withGM({ schema: { tags: ['GM'], security: [{ ApiKeyAuth: [] }] } }), ctrl.create.bind(ctrl));
  app.patch('/api/gm/sheets/:id', app.withGM({ schema: { tags: ['GM'], security: [{ ApiKeyAuth: [] }] } }), ctrl.update.bind(ctrl));
  app.delete('/api/gm/sheets/:id', app.withGM({ schema: { tags: ['GM'], security: [{ ApiKeyAuth: [] }] } }), ctrl.delete.bind(ctrl));
}
