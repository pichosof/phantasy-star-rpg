...
export async function gmImagesRoutes(app: FastifyInstance) {
  ...
  app.post('/api/gm/images', app.withGM({ schema: { tags: ['GM'], security: [{ ApiKeyAuth: [] }], consumes: ['multipart/form-data'] } }), ctrl.upload.bind(ctrl));
  app.delete('/api/gm/images/:id', app.withGM({ schema: { tags: ['GM'], security: [{ ApiKeyAuth: [] }] } }), ctrl.delete.bind(ctrl));
}
